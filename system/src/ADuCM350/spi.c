 ...
 * @version: $Revision: 33079 $
 * @date:    $Date: 2015-12-17 14:54:36 -0500 (Thu, 17 Dec 2015) $
 *****************************************************************************/
...
/*!
 * @brief  Turn HW Chip Select on or off.
 *
 * @param[in]    hDevice     Device handle obtained from adi_SPI_MasterInit() or adi_SPI_SlaveInit().
 * @param[in]    bFlag       Enable (true) or disable (false) HW Chip Select
 *
 * @return         Status
 *                - #ADI_SPI_ERR_BAD_DEV_HANDLE [D]         Invalid device handle parameter.
 *                - #ADI_SPI_ERR_NOT_INITIALIZED [D]        Device has not been previously configured for use.
 ...
 */
ADI_SPI_RESULT_TYPE  adi_SPI_SetHWChipSelect( ADI_SPI_DEV_HANDLE const hDevice, const bool_t bFlag)
{
#ifdef ADI_DEBUG
    if (ADI_SPI_INVALID_HANDLE(hDevice))
        return ADI_SPI_ERR_BAD_DEV_HANDLE;
 ...
 * buffer allocations due to the underlying SPI hardware that employs 16-bit DMA. PIO
 * mode transactions are limited to 64KB transactions.
...
ADI_SPI_RESULT_TYPE adi_SPI_MasterTransfer (ADI_SPI_DEV_HANDLE const hDevice, ADI_SPI_TRANSCEIVE_TYPE* const pXfr)
{
    ...
    /* At the start of a new transaction make sure SPICNT */
    /* does not hold a value from a previous transaction  */
    ...
    if (pXfr->PrologueSize) {
        ...
        /*
            Within the following prologue loop, the Transfer Initiate Mode
            pertains.
        
            In TIM mode 0 (default), the RX FIFO read initiates the serial
            transfer, so we sort of preload the transmit data into the TX FIFO,
            and launch it with an "fake" RX read to serialize the TX data.
        
            In TIM mode 1; the TX FIFO write initiates the serial transfer, so
            no need for "fake" an RX FIFO service during the transmit.
        
            So just always do the RX FIFO service and it covers both cases...
        */
        while (pXfr->PrologueSize) {
            ...
            /* (...also initiates serialization in TIM0 mode) */
            byte = hDevice->pSpi->SPIRX;

            /* wait until the rx fifi status reports non-zero content... */
    ...
    if (0 == pXfr->DataSize) {
...
#endif /* ADI_SPI_CFG_ENABLE_DMA_SUPPORT  */
        {
            ...
            /* Set the SPICNT to the number of bytes requested */
            /* Note: A zero SPICNT will lead to an extra byte  */
            /* being transmitted.                              */
            hDevice->pSpi->SPICNT = pXfr->DataSize;
...
static ADI_SPI_RESULT_TYPE AssertChipSelect(ADI_SPI_DEV_HANDLE const hDevice, const bool_t enable)
{
    ...
    if( hDevice->ChipSelect.HWCS == false )
    {
    ...
    uint32_t const Size)
{
    ...
    /* We need to clear the ALT channel bits just in case there was a previous transaction */
    /* that ended on the 'PING'. In this case the ALTSET will be indicating that the next  */
    /* transaction should start with the ALT DMA descriptor.                               */
    pADI_DMA->DMAALTCLR |=  (1 << hDevice->DMA_TX_Chan);
    pADI_DMA->DMAALTCLR |=  (1 << hDevice->DMA_RX_Chan);
